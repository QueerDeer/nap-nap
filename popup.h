...
protected:
    ...
    void mouseDoubleClickEvent (QMouseEvent *event); //Скипание отдыха даблкликом

public slots:
    void setPopupText(const QString& text); // Установка текста в уведомление
    void setPopupDuration(const int duration);    // Установка продолжительности показа
...
signals:
    void imHidden();                        //Сообщаем таймеру работы об окончании отдыха
...
private:
    ...
    QTimer *restTimer;          // Таймер, по которому виджет будет скрыт
    float popupOpacity;     // Свойства полупрозрачности виджета
    int time;               // Время демонстрации
};
